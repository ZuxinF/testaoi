...
def parse_args():
    ...
    parser.add_argument("--yolo-model", default=None, help="Optional YOLO segmentation model path, e.g. best.pt")
    parser.add_argument("--yolo-data-yaml", default=None, help="Optional YOLO data.yaml for mapping YOLO class ids to target class ids")
    parser.add_argument("--yolo-mask-dir", default=None, help="Optional existing YOLO pure prediction mask directory")
...
def find_mask_by_stem(mask_dir: Path | None, sample_id: str) -> Path | None:
    if mask_dir is None:
        return None
    for suffix in [".png", ".jpg", ".jpeg"]:
        path = mask_dir / f"{sample_id}{suffix}"
        if path.exists():
            return path
    return None
...
CLASS_ALIASES = {
    "administrative": ["administrative", "administration", "office", "business", "办公", "行政", "行政办公"],
    "commercial": ["commercial", "business", "商业"],
    "educational": ["educational", "education", "school", "教育", "学校"],
    "healthcare": ["healthcare", "medical", "hospital", "医疗", "医院"],
    "industrial": ["industrial", "industry", "factory", "工业", "工厂"],
    "public service": ["public service", "public", "government", "公共服务", "公共", "政府"],
    "residential": ["residential", "dense residential", "居住", "住宅"],
    "sport and art": ["sport and art", "sport", "art", "recreation", "resort", "体育", "文化", "文体"],
    "unknown": ["unknown", "未知"],
    "others": ["others", "other", "其他"],
}


def load_yolo_class_names(path: str | None) -> list[str] | None:
    if not path:
        return None
    data_yaml = Path(path)
...
def build_yolo_to_target_map(yolo_names: list[str] | None, target_names: list[str]) -> dict[int, int]:
    normalized_to_target = {normalize_class_name(name): idx for idx, name in enumerate(target_names)}
    alias_to_target: dict[str, int] = {}
    for canonical, aliases in CLASS_ALIASES.items():
        target_id = normalized_to_target.get(canonical)
        if target_id is None:
            for alias in aliases:
                target_id = normalized_to_target.get(normalize_class_name(alias))
                if target_id is not None:
                    break
        if target_id is None:
            continue
        for alias in aliases:
            alias_to_target[normalize_class_name(alias)] = target_id

    if not yolo_names:
        # Fallback for datasets generated by prepare_yolo_seg_from_tiles where
        # YOLO class ids usually equal target class ids minus background.
        return {idx: idx + 1 for idx in range(max(len(target_names) - 1, 0))}

    mapping: dict[int, int] = {}
    for yolo_id, name in enumerate(yolo_names):
        normalized = normalize_class_name(name)
        if normalized in normalized_to_target:
            mapping[yolo_id] = normalized_to_target[normalized]
        elif normalized in alias_to_target:
            mapping[yolo_id] = alias_to_target[normalized]
        elif yolo_id + 1 < len(target_names):
            mapping[yolo_id] = yolo_id + 1
        else:
            mapping[yolo_id] = 0
    return mapping


def yolo_result_to_mask(result, size: tuple[int, int], yolo_to_target: dict[int, int]) -> np.ndarray:
    mask_out = np.zeros((size[1], size[0]), dtype=np.uint8)
    if result.masks is None:
        return mask_out
    masks = result.masks.data.detach().cpu().numpy()
    if result.boxes is not None and result.boxes.cls is not None:
        class_ids = result.boxes.cls.detach().cpu().numpy().astype(np.uint8).tolist()
    else:
        class_ids = [0] * len(masks)
    for mask, class_id in zip(masks, class_ids):
        if mask.shape != mask_out.shape:
            mask_img = Image.fromarray((mask > 0.5).astype(np.uint8) * 255)
            mask = np.asarray(mask_img.resize(size, Image.NEAREST)) > 0
        else:
            mask = mask > 0.5
        mask_out[mask] = int(yolo_to_target.get(int(class_id), 0))
    return mask_out
...
def load_yolo_model(path: str | None):
    ...
    try:
        ...
        raise RuntimeError("Using --yolo-model requires ultralytics. Please run in the YOLO environment or install ultralytics.") from exc
    return YOLO(path)
...
def make_yolo_mask(
    ...
    image: Image.Image,
    model,
    yolo_mask_dir: Path | None,
    yolo_imgsz: int,
    yolo_conf: float,
    yolo_device,
    yolo_to_target: dict[int, int],
) -> np.ndarray | None:
    existing_mask = find_mask_by_stem(yolo_mask_dir, sample_id)
    if existing_mask:
        arr = np.asarray(Image.open(existing_mask), dtype=np.uint8)
        if arr.ndim == 3:
            arr = arr[..., 0]
        return arr
    if model is None or image_path is None or not image_path.exists():
        return None
    ...
    return yolo_result_to_mask(results[0], image.size, yolo_to_target)
...
def write_html(out_dir: Path, title: str, records: list[dict], class_names: list[str], summary: dict):
    ...
    for rec in records:
      ...
      <article>
        ...
        {f"<p>YOLO类别：{format_class_count(rec['yolo_classes'])}</p>" if rec.get("yolo_mask_file") else ""}
        <div class="grid">
          ...
          {f'<figure><img src="{html.escape(rec["yolo_mask_file"])}"><figcaption>YOLO预测 mask</figcaption></figure>' if rec.get('yolo_mask_file') else ''}
...
<head>
  ...
  <style>
    ...
    .grid {{ display: grid; grid-template-columns: repeat({6 if summary.get('has_yolo') else 5}, minmax(0, 1fr)); gap: 8px; }}
...
def main():
    ...
    yolo_names = load_yolo_class_names(args.yolo_data_yaml)
    yolo_to_target = build_yolo_to_target_map(yolo_names, class_names)
    yolo_model = load_yolo_model(args.yolo_model)
    yolo_mask_dir = Path(args.yolo_mask_dir) if args.yolo_mask_dir else None
    ...
    for row in filtered:
        ...
        yolo_mask_file = f"yolo_mask_{sample_id}.png"
        ...
        yolo_mask = make_yolo_mask(
            sample_id=sample_id,
            image_path=image_path,
            image=image,
            model=yolo_model,
            yolo_mask_dir=yolo_mask_dir,
            yolo_imgsz=args.yolo_imgsz,
            yolo_conf=args.yolo_conf,
            yolo_device=args.yolo_device,
            yolo_to_target=yolo_to_target,
        )
        if yolo_mask is not None:
            colorize(yolo_mask).save(out_dir / yolo_mask_file)
        ...
        records.append(
            {
                ...
                "yolo_mask_file": yolo_mask_file if yolo_mask is not None else None,
                "gt_mask_file": gt_mask_file,
                "error_file": error_file,
                "gt_classes": class_count(gt, class_names),
                "pred_classes": class_count(pred, class_names),
                "yolo_classes": class_count(yolo_mask, class_names) if yolo_mask is not None else [],
            }
        )
    ...
    summary = {
        ...
        "has_yolo": any(r.get("yolo_mask_file") for r in records),
        "yolo_model": args.yolo_model,
        "yolo_data_yaml": args.yolo_data_yaml,
        "yolo_names": yolo_names,
        "yolo_to_target": yolo_to_target,
        "yolo_mask_dir": str(yolo_mask_dir) if yolo_mask_dir else None,
    }
